<template>
  <view class="single-match-confirm-page">
    ...
    <!-- 下划线图片 - 置于页面顶部 -->
    <view class="underline-section">
      <image src="/static/match-mutiple-part1/下划线.png" class="underline-image" mode="aspectFit" />
    </view>
...
export default {
  ...
  methods: {
    ...
    startCheckin() {
      ...
      // 检查是否有队名，如果没有队名，跳转到创建队名页面
      const teamName = uni.getStorageSync('teamName')
      if (!teamName || !teamName.trim()) {
        // 没有队名，跳转到创建队名页面
        // 只使用 navigateTo，不使用 reLaunch，避免清空页面栈
        uni.navigateTo({
          url: '/pages/team-name/index',
          fail: (err) => {
            console.warn('跳转到 team-name 页面失败:', err)
            uni.showToast({
              title: '跳转失败，请重试',
              icon: 'none'
            })
...
.info-card {
  ...
  padding: 20rpx 20rpx 32rpx; /* 减少顶部padding，使文字上移 */
...
.info-row {
  ...
  justify-content: space-between;
  align-items: center;
  margin-bottom: 50rpx; /* 增大间距从32rpx到40rpx */
  padding: 0; /* 移除padding，使文字上移 */
  margin-top: -10rpx; /* 负margin使文字整体上移 */
}

.info-row:first-child {
  margin-top: 0; /* 第一行不需要负margin */
}
